    ...
    try {
      ...
      if (data) {
        const apiTeams = data.map((p) => {
          return {
            id: p.batch_id
              ? `B${p.batch_id.toString().padStart(3, '0')}`
              : p.id.toString(),
            ...
            // NEW: IIC Co-ordinator
            iic_coordinator: p.iic_coordinator_assigned || 'N/A',
            ...
            status:
              (p.review_status || '').toLowerCase() === 'completed'
                ? 'Completed'
                : 'Pending',
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (newStatus === "Completed") {
          alert(
            "Status changed and Certificate Email sent to Team Lead successfully!"
          );
        }
      } else {
        ...
        alert(
          `Failed to update status: ${errorData.message}`
        );
      }
    } catch (error) {
              ...
              filteredData.map((team) => (
                ...
                >
                  ...
                  <td data-label="Register Number">
                    {team.register_number}
                  </td>

                  {/* NEW: Register Mail */}
                  <td data-label="Registered Mail">
                    {team.email}
                  </td>
